...
```
a && b   ==>   %t = alloca i1
               <eval a>;  br a_nonzero, bb_rhs, bb_false
   bb_rhs:     <eval b>;  store (b != 0) -> %t;  goto bb_join
   bb_false:   store 0 -> %t;                    goto bb_join
   bb_join:    %r = load i1, %t
```

An `alloca` rather than a phi, because CIR is not SSA ([020 §1.3](020-cir.md)). `||` is
the mirror image. The conditional operator `?:` uses the same shape with both arms
evaluated into the slot.
...
Every compound statement gets a `ScopeId`. Lowering emits `Marker(Scope { scope, Enter })`
on entry and `Scope { scope, Exit }` on **every** exit path — falling off the end,
`break`, `continue`, `goto` out, and `return`. A missed exit marker leaks a stack object
and produces a false use-after-scope later, so this is the most error-prone part of
lowering and is pinned by contracts 9–11.
...
> the distinct `(file, line)` pairs whose file is the block's own TU. `gcov_lines` is that
> set, sorted ascending.

Four consequences, each of which is a way to get it wrong:
...
- **Lines from other files are dropped**, not remapped. After `expansion_loc` the location
  is in the expanding TU by construction; anything else indicates a `#line` directive or
  an inlined header function, and those are recorded on the `Inst` span, not here.
...
explicitly-initialized members — not a member-by-member store of every field, so partial
initialization stays visible to the uninitialized-read checker.
...
17. Over the corpus, the union of all blocks' `gcov_lines` for a function is a **subset**
    of the lines gcov reports for that function — chiero must not claim a line gcov does
    not attribute there.
18. `switch` with fallthrough lowers to blocks chained by `Goto`, and a `case` range of 4
    expands to 4 sorted cases.
19. An automatic aggregate with a partial initializer leaves the uninitialized members
    detectable as uninitialized (no blanket "fully initialized" marking).
