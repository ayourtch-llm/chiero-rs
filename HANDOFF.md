...
>
> Same rule at three scales. Any one collapsing makes the published count worthless:
>
> | scale | mechanism |
> |---|---|
> | selector | `Selection::NeedsAst` — **do not collapse to a bool when the AST lands** |
> | recipe | `RecipeTally::needs_ast` + `is_complete()` |
> | sweep | `Tier1Report::unreadable` + `is_complete()` |
>
> It earned its keep on first contact: a `recipe-sweep` run with no `-I` reported *36 unreadable,
> PARTIAL* rather than `0 candidates`, which would have read as a clean tree.
>
> ### The function list: two fixes, and why both were needed
>
> `vnet/fib` reported **186,623 functions from 36 TUs**. Fixing attribution to the defining file
> **did not change the number at all** — every TU still contributed a copy, now carrying the
> header's path. Deduplicating on `(defining file, name)` gave **6,903** and halved the runtime.
> **A precondition fix that leaves the symptom identical is not the fix**; re-measure before
> claiming one.
>
> Attribution uses **`expansion_loc`, not `lookup_loc`**: a macro-generated function is written
> by the file that *invokes* the macro, and its raw token position is the header holding the
> macro body. VPP generates node functions this way constantly, so `lookup_loc` files every node
> under one header. The first fixture had no macros and passed under both readings.
>
> ### ⚠️ Recipe evaluation is absent, not stubbed — keep it that way
>
> A `good` fixture passes exactly when it produces **no** finding, so a null evaluator satisfies
> 042 c2 for every recipe while covering nothing. Unread `track`/`require` clauses are kept whole
> as `UnparsedClause` with their **count asserted**.
>
> Two recipes are vendored in `tests/corpus/recipes/`: one structural and evaluable today, one
> the 042 §4 CLI recipe waiting on the frontend, so both paths are exercised.
>
...
>
> * `chiero-recipe` tier-1 structural sweep (stub); VPP parser-coverage percentage published.
> * `explain_macro_expansion` has no column-less way to ask for *one* independent call among
>   several on a line; it returns them all, which is now the documented behaviour.
>
> ### The `vnet` sweep landed and overturned the convergence claim
>
> 452 TUs, 348 findings. **345 are the two known pedantic categories.** Of the three new
> kinds, two (`struct has no members`, `pointer from an integer without a cast`) are also
> gnu11-accepted / pedantic-refused and join the calibration set. The third was a **regression
> I introduced in wave 408** and did not detect for three waves — see below.
>
> ### ⚠️ The wave-408 regression, and why the tests missed it
>
> Wave 408 held a token chunk across a directive while a `(` was open. `has_open_paren` cannot
> tell a macro's argument list from ordinary C, and VPP's X-macro accumulator opens one:
>
> ```c
> const u8 MASK = (
...
> ```
>
> The deferral carried the uses past the `#undef`, so `_` was gone when they expanded. Fixed by
> `in_open_macro_args`, which tracks **per unclosed paren** whether a function-like macro name
> opened it.
>
> **Every wave-408 fixture put a macro name immediately before the `(`, so all of them agreed
> with both the correct rule and the wrong one.** A predicate is only tested by an input where
> the candidate rules *disagree*. Mutation missed it too: the mutants attacked whether the
> deferral happened, never its criterion.
>
> ### Method notes — read these before trusting a green suite
>
> * **A justification is a claim; a claim in a commit message is not evidence.** Three times in
>   this stretch prose asserted an unmeasured fact. Twice merely untested (the inner frame's
>   call site; the trailing-macro-name rule); once **false** (argument spans do not carry
>   whitespace, so that `.trim()` was dead, not load-bearing).
> * **A surviving mutant has four readings**, in order: the code is wrong; a row is missing; the
>   code is dead; the mutant is equivalent. The trailing-macro-name survivor was reading one —
>   `gcc -E` and `clang -E` both leave `int v = P` / `#define K 5` / `(1);` unexpanded, so the
>   *mutant* was correct. **Check a survivor against the oracle before writing it off.**
> * **Check the input reaches the line** before calling a mutant equivalent; one survivor was an
>   early return, another a "mutant" whose replacement equalled the original.
> * A deliberate equivalent mutant is allowed if recorded: the chain sort is unobservable under
>   today's expansion table and kept as an ordering promise, with the *guarantee* covered by a
>   three-item test even though the line is not.
> * **`Span::DUMMY` resolves** to line 1 col 1 of whichever file starts at offset 0. Reject by name.
> * **A report that names only *what* is not actionable**: `describe`/`kind` turned "somewhere in
>   452 files" into three exact lines, and all three reduced immediately.
>
> ### The owner's queue is two clean questions
>
> | item | reach | chiero today |
> |---|---|---|
> | **pedantic mode for sema** | 345 of 348 `vnet` findings | verdicts correct, noise on the corpus |
...
> which is true of 014 and a different thing from unconstrained. The rule went into
> `chiero-parse`, and the RED announced the layering by failing on the sema harness's parse-clean
> assertion. **When a census row's tokens never reach 014, the rule belongs to 013** — check where
> the information dies before choosing the crate.
>
...
> **false positive** (a tag defined in a parameter list called a redefinition, and reported *twice*
> for a definition), three misses, and a duplicate. The cause of the false positive was that a
> parameter list was not a scope; the cause of one miss was the same mis-keying wave 358 found, in
> a different rule. **Two waves running, the largest finding was a check that existed and was
> keyed on the wrong thing.** Ask that question first.
>
...
> The RED said so by failing on the sema harness's parse-clean assertion rather than on its own
> assertion — read that failure as "wrong crate", not as a broken harness.
>
> **When two paragraphs describe one mistake, gcc's message picks the survivor** (wave 366,
> applying 361). `struct S { int a[]; int b; }` violates "needs a member before it" *and* "must be
> last"; gcc says only the second. The fix was to move the new check **after** the loop, where
> "is it the only member" is answerable — a question the loop cannot answer while running.
>
...
> SURVIVED. Wave 357's rule covers mutants that never applied; this is the neighbour, a mutant that
> applied and modelled the wrong thing. For any paired edit (enter/leave, push/pop, save/restore),
> mutate **both halves or neither**, and treat a survivor on a rule you just watched fire as a bug
> in the mutant until proven otherwise.
>
> **A scope is the fix for a whole class of false positive** (wave 359). A parameter list was not
> one, so a tag defined in it was a redefinition of itself — twice over for a definition, which
> resolves the list a second time. When something is reported *twice*, look for a pass that runs
> twice before looking at the rule.
>
> **A fixture written to prove you broke nothing can find something** (wave 359). `enum E { A };
> enum E { B };` was in the new test only to show genuine redefinitions still reported. They never
> had: the struct path used the scoped set and `enum_ty` did not. **Write the "still rejected" half
> even when you are certain**, and read its failure as a finding rather than as a bad assertion.
>
