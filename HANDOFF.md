...
| **a new subsystem: all of `vnet/`, 423 entries** (only `vnet/ip/` had ever been swept) | one sweep, ~25 min | **44 findings, 0 `Exact`, 0 `timeout`, 0 `noinc`** — and two `failed` rows that are **real defects in VPP source the build never compiles**: a function defined twice, and a call with no declaration. gcc agrees on both. The harness lesson is bigger than either: `pick_entries.py` globs the tree, so `failed` mixes "chiero cannot read this" with "nothing can" |
