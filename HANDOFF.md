...
| `prove-equivalent` | `differs` verdict, always | `replay: (none)` — printed directly above a blind spot saying, in words, that no harness was compiled |
...
> | 2 | **Language level — `__STDC_VERSION__` `201112L` or `201710L`?** (`1d`) VPP builds with no `-std=`, so gcc's default gnu17 applies and every glibc header configures for C17; chiero says C11 | a persona edit | ✅ **measured 2026-08-10, and it changes nothing observable.** Every `__STDC_VERSION__` comparison in `/usr/include` and gcc's own headers is `> 201710L` (25 — C23 branches, false for both), `>= 199901L` (17, true for both), `>= 201112L` (9, true for both), `<= 201710L` (3, true for both), `< 199901L` (1, false for both) or `>= 199409L` (1, true for both); **VPP's entire tree has one use, `< 199901L`, false for both.** No branch on this machine or in that checkout distinguishes the two values. C17 is a bug-fix release with no new features over C11, so a parser that handles C11 handles C17 — the change is a one-line honesty edit with no measured risk. Recommend `201710L`; still the owner's word to say |
...
> | 6 | **011 contract 12 — a throughput floor the code does not meet, and no gate that would say so.** It reads *"≥100 MB/s lexing over a 50 MB blob"*. 🔴 **Measured 2026-08-10: 70.5 / 71.6 / 72.0 MB/s in `--release`** — the configuration the test's own doc comment names — so the contract is **failing by ~29%**, and 49.7 / 50.2 MB/s in the default profile. Nobody knew, because its only test is `#[ignore]`d and deliberately cites no contract number ("ignored tests do not carry contract coverage credit"), which is why it is the one uncited M2 contract. ⚠️ It is *also* ill formed by this project's own rule — *"a counter, not a clock: a wall-clock bound silently stops being able to fail whenever the build gets faster"* — but the sharper fact is that it stopped being able to **pass** and nothing noticed | a spec edit, then a test; `growth.rs` already has the shape | amend it to a **ratio per 4x input** (precedent next door in 011 c13) **and take the absolute number out**, rather than either loosening 100 to 70 or optimising the lexer to reach a number chosen on no evidence. ⏭️ Whichever way it goes, the test must stop being `#[ignore]`d — the third time today the answer was *nobody ran the gate* |
...
6a. 🔶 **HALF DONE 2026-08-11 — `chiero serve` speaks JSON-RPC and contract 18's check runs; the MCP handshake does not exist.** Was: **The MCP/JSON-RPC server — the one thing 080 says M7 is missing, and it is not on this
   ...
   anywhere on disk … every detail would be written from memory and tested against nothing"* —
   and that was **the fifth false impossibility claim of 2026-08-11 and the second of my own**.
   The machine has no schema *on disk* and it has a network:

   | probe | answer |
   |---|---|
   | `curl …/modelcontextprotocol/main/schema/2025-06-18/schema.json` | **HTTP 200, 108 KB** |
   | `python3 -c "import jsonschema"` | **4.10.3, installed** |

   So the shapes I said were unknowable are simply readable: `InitializeResult` requires
   `capabilities`, `protocolVersion`, `serverInfo`; `CallToolResult` requires `content` (with
