...
| `./check.sh` | ✅ committed — the green gate; ⚠️ **it reports a failing suite, not the failing assertion**, which cost a diagnosis on 2026-08-09 when a solver test flaked once and could not be looked at afterwards. Re-running the named suite is the workaround and usually enough; keys on cargo's **exit status**, prints failing suites first (§7.5). ⚠️ **Widened 2026-08-07 to all three CI legs** — it ran `cargo test` alone while CI also gates `cargo fmt --all --check` and `clippy -D warnings`, and was reporting GREEN over **26 fmt diffs and 2 clippy errors**. Fast legs run first, in seconds, so a formatting diff is not found after the hour. `--skip-lints` re-runs the tests alone. **Still not covered, and named in the file:** CI's second solver leg |
