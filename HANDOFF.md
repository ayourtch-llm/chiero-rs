...
| `tests/corpus/handoff/numbering.py` | ✅ committed 2026-08-09 — **a checker for this file.** Numbered lists must not repeat or go backwards; §9's START HERE had two items numbered `3.` after thirty-five waves of incremental edits. Exits 1 on an anomaly, no dependency, ~50 lines. ⚠️ Verified against the *broken* revision (`HEAD~1` of the repair reports line 1793) as well as the fixed one. **Not a proof the prose is current** — one cheap symptom of drift, and its docstring says so. ✅ **`./check.sh` runs it**, the one leg broader than CI, because before pushing is when a broken handoff would go out |
